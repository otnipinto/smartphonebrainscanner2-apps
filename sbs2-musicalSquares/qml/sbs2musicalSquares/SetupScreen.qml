...
Rectangle {
   // anchors.fill: parent
    ...
    property string userText: user.text
    property string descTest: description.text
    property double baselineValue: parseFloat(baseline.text)
    ...
    Button {id: quit; desc: "quit"; y: 80; x: page.width-quit.width; isEnabled: 1}
    Button {id: start; desc: "start"; y: 330; x: 0; isEnabled: {
								if (page.baselineRecording) return 1;
								if (baseline.text != "" && !isNaN(baseline.text)) return 1;
								return 0;
								}}
	...
	MouseArea
	{
		...
		page.baselineRecording = parent.turnedOn;
	    }
	}
    }

    MyTextInput {id: baseline; desc: "baseline"; x: page.width/2 - 80; anchors.verticalCenter: description.verticalCenter; visible: !baselineSwitch.turnedOn; }
