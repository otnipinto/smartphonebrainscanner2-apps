    ...
    Sbs2Callback(parent), glwidget(glwidget_)
{
    ...
    sbs2Region = new Sbs2Region();
    sbs2Region->addRegion("brodmann_area_4");
    ...
    updateSimulationTimer = new QTimer(this);
    QObject::connect(updateSimulationTimer,SIGNAL(timeout()),this,SLOT(updateSimulation()));
    updateSimulationTimer->start(250);

    simulationLimit = 4 * 60 * 1;
    simulationFrames = 0;

}

void MyCallback::updateSimulation()
{
    soureReconstructionPowerReady();
}
...
void MyCallback::soureReconstructionPowerReady()
{
    ...
    createColorMatrix3();
    glwidget->updateValues(currentPowerValue1,currentPowerValue2);
    updateModel();
}

void MyCallback::sourceReconstructionReady()
{
    createColorMatrix2(sbs2DataHandler->getSourceReconstructionMeanValues());
    updateModel();
}
...
void MyCallback::createColorMatrix3()
{
    currentPowerValue1 = (simulationFrames/(double)simulationLimit) + (1-(qrand()/(double)RAND_MAX)*2)*0.02;

    currentPowerValue2 = (simulationFrames/(double)simulationLimit)*1.5 + (1-(qrand()/(double)RAND_MAX)*2)*0.02;


    ++simulationFrames;


    if (simulationFrames > simulationLimit)
	simulationFrames = 0;

    for (int vertex = 0; vertex<colorData->dim1(); ++vertex)
    {

	if (!sbs2Region->getVerticesToExtract()->contains(vertex))
	{
	    (*colorData)[vertex][0] = 0.8;
	    (*colorData)[vertex][1] = 0.8;
	    (*colorData)[vertex][2] = 0.8;
	    (*colorData)[vertex][3] = 0.8;

	}
	else
	{

	    (*colorData)[vertex][0] = 1.0;
	    (*colorData)[vertex][1] = 1-currentPowerValue1;
	    (*colorData)[vertex][2] = 1-currentPowerValue1;
	    (*colorData)[vertex][3] = 1.0;
	}
    }
